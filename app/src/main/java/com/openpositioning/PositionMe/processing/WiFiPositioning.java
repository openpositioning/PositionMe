...
public class WiFiPositioning {
  ...
  /**
   * Creates a POST request using the WiFi fingerprint to obtain user's location The POST request is
   * issued to https://openpositioning.org/api/position/fine (the openpositioning API) with the WiFI
   * fingerprint passed as the parameter.
   * <p>
   * The response of the post request returns the coordinates of the WiFi position along with the
   * floor of the building the user is at.
   * <p>
          ...
          // Other Errors
            ...
            if (error.networkResponse != null) {
              Log.e("WiFiPositioning",
                  "Response Code: " + error.networkResponse.statusCode + ", " + error.getMessage());
            } else {
   ...
   * <p>
   * The response of the post request returns the coordinates of the WiFi position along with the
   * floor of the building the user is at though a callback.
   * <p>
   * A try and catch block along with error Logs have been added to keep a record of error's
   * obtained while handling POST requests (for better maintainability and secure programming)
   *
