...
/**
 * Processes data from the sensors and estimates the position of the user using PDR (Pedestrian
 * Dead Reckoning) algorithms. Implements the {@link SensorDataListener} interface to
 * receive data from a SensorHub instance. Implements {@link Observable} to notify
 * observers of changes to processed PDR data.
 * Original Authors
 * @author Mate Stodulka
 * @author Michal Dvorak
 * Updated by:
 * @author Philip Heptonstall. Refactored to follow single-responsibility principles, and interface
 * with sensors directly as a SensorDataListener.
 */
public class PdrProcessing implements SensorDataListener<SensorData>, Observable {
  ...
  /**
   * Function to calculate PDR coordinates from sensor values. Should be called from the step
   * detector sensor's event with the sensor values since the last step.
   *
   ...
   */
  public void updatePdr(long currentStepEnd, List<Double> accelMagnitudeOvertime,
      float headingRad) {
    if (accelMagnitudeOvertime == null || accelMagnitudeOvertime.size() < MIN_REQUIRED_SAMPLES) {
      return;  // Return current position without update
    }
  ...
  /**
   * Calculates the relative elevation compared to the start position. The start elevation is the
   * median of the first three seconds of data to give the sensor time to settle. The sea level is
   * irrelevant as only values relative to the initial position are reported.
   *
  ...
  /**
   * Get the current X and Y coordinates from the PDR processing class. The coordinates are in
   * meters, the start of the recording is the (0,0)
   *
  ...
  /**
   * Estimates if the user is currently taking an elevator. From the gravity and gravity-removed
   * acceleration values the magnitude of horizontal and vertical acceleration is calculated and
   * stored over time. Averaging these values and comparing with the thresholds set for this class,
   * it estimates if the current movement matches what is expected from an elevator ride.
   *
  ...
  public boolean estimateElevator(float[] gravity, float[] acc) {
    ...
    float horizontalAcc = (float) Math.sqrt(
        Math.pow(acc[0] - verticalAcc * gravity[0] / g, 2)
            + Math.pow(acc[1] - verticalAcc * gravity[1] / g, 2)
            + Math.pow(acc[2] - verticalAcc * gravity[2] / g, 2));
    ...
    if (this.verticalAccel.isFull() && this.horizontalAccel.isFull()) {
      ...
      float horizontalAvg =
          optHorizontalAvg.isPresent() ? (float) optHorizontalAvg.getAsDouble() : 0;
  ...
  /**
   * Resets all values stored in the PDR function and re-initialises all buffers. Used to reset to
   * zero position and remove existing history.
   */
...
//            this.notifyObservers(0);
  ...
  /**
   * Record class to hold PDR updates to send to observers.
   *
   * @param position estimated PDR position in meters.
   * @param newPosition true if the position has changed, false otherwise.
   * @param inElevator true if the user is in an elevator, false otherwise.
   * @param newElevator true if the elevator status has changed, false otherwise.
   */
  ...
  /**
   * Notify observers of changes to processed PDR data, in either the X,Y, estimates or elevator
   * status.
   */
  @Override
