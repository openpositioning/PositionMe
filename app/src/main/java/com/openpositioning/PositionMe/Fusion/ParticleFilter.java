...
import static com.openpositioning.PositionMe.Fusion.FusionConstants.*;

/**
 * Particle filter for indoor position estimation.
 *
 * <p>Maintains a population of weighted particles in a local East-North coordinate frame
 * and uses sequential Monte Carlo methods to estimate the user's position. Particles are
 * propagated using PDR displacement and resampled using systematic resampling when weight
...
public class ParticleFilter {
     ...
     *
     * <p>Sets the WGS84 reference point for coordinate conversion and distributes particles
     * with Gaussian uncertainty around the initial position.
     *
     ...
     *
     * <p>After propagation, weights are updated and systematic resampling is triggered
     * if weight degeneracy exceeds the threshold. The position estimate is updated each cycle.
     *
    ...
    public LatLng enToLatLng(double easting, double northing) {
        ...
        double lng = refLng + easting / (METRES_PER_DEG_LNG_AT_EQUATOR * Math.cos(Math.toRadians(refLat)));
     ...
     *
     * <p>Builds a cumulative weight distribution and selects particles at evenly spaced
     * intervals with a single random offset. Selected particles are duplicated with a small
     * Gaussian jitter to maintain diversity, and weights are reset to uniform.
     *
