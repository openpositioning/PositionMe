 ...
 *
 * <p>The class implements {@link Observable} for informing {@link Observer} classes of updated
 ...
 *
 * <p>The class ensures all required permissions are granted before enabling the Wi-Fi. The class
 * will periodically start a wifi scan as determined by {@link SensorFusion}. When a broadcast is
 * received it will collect a list of users and notify users. The {@link
 * WifiDataProcessor#getCurrentWifiData()} function will return information about the current Wi-Fi
 * when called by {@link SensorFusion}.
 *
...
public class WifiDataProcessor implements Observable {
    ...
    public WifiDataProcessor(Context context) {
        ...
        //      //  if(permissionsGranted && wifiManager.getWifiState()==
    ...
    /**
     * Broadcast receiver to receive updates from the wifi manager. Receives updates when a wifi
     * scan is complete. Observers are notified when the broadcast is received to update the list of
     * wifis
     */
    BroadcastReceiver wifiScanReceiver =
            new BroadcastReceiver() {
                /**
                 * Updates the list of nearby wifis when the broadcast is received. Ensures wifi
                 * scans are not enabled if permissions are not granted. The list of wifis is then
                 * passed to store the Mac Address and strength and observers of the
                 * WifiDataProcessor class are notified of the updated wifi list.
                 *
                 * @param context Application Context to be used for permissions and device
                 *     accesses.
                 * @param intent ???.
                 */
                @Override
            ...
            if (macByte != ':') {
                ...
                if ((int) macByte >= 48 && (int) macByte <= 57) {
                    intMacAddress =
                            intMacAddress
                                    + (((int) macByte - 48)
                                            * ((long) Math.pow(16, 16 - j - colonCount)));
                }
    ...
    /**
     * Checks if the user authorised all permissions necessary for accessing wifi data. Explicit
     * user permissions must be granted for android sdk version 23 and above. This function checks
     * which permissions are granted, and returns their conjunction.
     *
     * @return boolean true if all permissions are granted for wifi access, false otherwise.
     */
    private boolean checkWifiPermissions() {
        int wifiAccessPermission =
                ActivityCompat.checkSelfPermission(
                        this.context, Manifest.permission.ACCESS_WIFI_STATE);
        int wifiChangePermission =
                ActivityCompat.checkSelfPermission(
                        this.context, Manifest.permission.CHANGE_WIFI_STATE);
    ...
    /**
     * Scan for nearby networks. The method checks for permissions again, and then requests a scan
     * of nearby wifis. A broadcast receiver is registered to be called when the scan is complete.
     */
    ...
    /**
     * Initiate scans for nearby networks every 5 seconds. The method declares a new timer instance
     * to schedule a scan for nearby wifis every 5 seconds.
     */
    ...
    /**
     * Cancel wifi scans. The method unregisters the broadcast receiver associated with the wifi
     * scans and cancels the timer so that new scans are not initiated.
     */
            ...
            if (android.os.Build.VERSION.SDK_INT >= android.os.Build.VERSION_CODES.R) {
                ...
                isThrottled =
                        Settings.Global.getInt(
                                        context.getContentResolver(), "wifi_scan_throttle_enabled")
                                == 1;
            }
     ...
     *
     * <p>Implements default method in {@link TimerTask} class which it implements. It begins to
     * start calling wifi scans every 5 seconds.
     */
     ...
     *
     * <p>A connectivity manager is used to obtain information about the current network. If the
     * device is connected to a network its ssid, mac address and frequency is stored to a Wifi
     * object so that it can be accessed by the caller of the method
     *
