...
public class SensorInfoListAdapter extends RecyclerView.Adapter<SensorInfoViewHolder> {
    ...
    public SensorInfoViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        return new SensorInfoViewHolder(
                LayoutInflater.from(context)
                        .inflate(R.layout.item_sensorinfo_card_view, parent, false));
    }

    /**
     * {@inheritDoc} Formats and assigns the data fields from the SensorInfo object to the TextView
     * fields.
     *
    ...
    public void onBindViewHolder(@NonNull SensorInfoViewHolder holder, int position) {
        ...
        String vendorString =
                context.getString(R.string.vendor, sensorInfoList.get(position).getVendor());
        holder.vendor.setText(vendorString);

        String resolutionString =
                context.getString(
                        R.string.resolution,
                        String.format("%.03g", sensorInfoList.get(position).getResolution()));
        holder.resolution.setText(resolutionString);
        String powerString =
                context.getString(
                        R.string.power,
                        Objects.toString(sensorInfoList.get(position).getPower(), "N/A"));
        holder.power.setText(powerString);
        String versionString =
                context.getString(
                        R.string.version,
                        Objects.toString(sensorInfoList.get(position).getVersion(), "N/A"));
        holder.version.setText(versionString);
    }

    /**
     * {@inheritDoc} Number of SensorInfo objects.
     *
     * @see SensorInfo
     */
    @Override
