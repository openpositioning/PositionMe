...
/**
 * Processes data recorded in the {@link SensorFusion} class and calculates live PDR estimates. It
 * calculates the position from the steps and directions detected, using either estimated values
...
public class PdrProcessing {
     ...
     */
    public float[] updatePdr(
            long currentStepEnd, List<Double> accelMagnitudeOvertime, float headingRad) {
    ...
    private float weibergMinMax(List<Double> accelMagnitude) {
        ...
        List<Double> validAccel =
                accelMagnitude.stream().filter(Objects::nonNull).collect(Collectors.toList());
     ...
     * stored over time. Averaging these values and comparing with the thresholds set for this
     * class, it estimates if the current movement matches what is expected from an elevator ride.
     *
    ...
    public boolean estimateElevator(float[] gravity, float[] acc) {
        ...
        if (this.verticalAccel.isFull() && this.horizontalAccel.isFull()) {
            ...
            OptionalDouble optVerticalAvg =
                    verticalMemory.stream().mapToDouble(Math::abs).average();
            float verticalAvg =
                    optVerticalAvg.isPresent() ? (float) optVerticalAvg.getAsDouble() : 0;
            ...
            OptionalDouble optHorizontalAvg =
                    horizontalMemory.stream().mapToDouble(Math::abs).average();
            float horizontalAvg =
