 ...
 *
 * <p>This class: - Manages the permissions list. - Checks if all permissions are granted. -
 * Requests missing permissions. - Handles both the first-time and permanent denial scenarios.
 *
 * <p>Usage from MainActivity: PermissionManager permissionManager = new
 * PermissionManager(MainActivity.this, new PermissionManager.PermissionCallback() { @Override
 * public void onAllPermissionsGranted() { // e.g. call allPermissionsObtained() in MainActivity
 * allPermissionsObtained(); } }); permissionManager.checkAndRequestPermissions();
 */
public class PermissionManager {
    ...
    public void checkAndRequestPermissions() {
        if (!hasAllPermissions()) {
            ActivityCompat.requestPermissions(
                    activity, requiredPermissions.toArray(new String[0]), ALL_PERMISSIONS_REQUEST);
        } else {
            // Already granted
    ...
    /**
     * Must be called from the Activity's onRequestPermissionsResult: @Override public void
